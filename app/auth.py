...
telegram_secret_token_query = APIKeyHeader(name="X-Telegram-Bot-Api-Secret-Token", auto_error=False)
...
def verify_telegram_api_header(api_header_query: str = Security(telegram_secret_token_query)):
    if api_header_query is None or not secrets.compare_digest(api_header_query, TELEGRAM_BOT_SECRET_TOKEN):
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail="API Key Invalid"
        )
