...
from .config import ALL_SCRAPER, ALL_SINGLE_SCRAPER, X_RAPIDAPI_HOST, SCRAPER_INFO, SHORT_LIMIT
...
class Twitter(MetadataItem):
        ...
        for scraper in scrapers:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    continue
        return {"errors": "No valid response."}
    ...
    def process_tweet_Twitter135(self, tweet_data: Dict):
        ...
        tweets = []
        for i in entries:
            if i['content']['entryType'] == 'TimelineTimelineItem' and i['content']['itemContent'][
        ...
        if tweet['media']:
            for media in tweet['media']:
                if media['type'] == 'photo':
                    ...
                    tweet_info[
                        'content_group'] += f'<video controls="controls" src="{highest_bitrate_item["url"]}"></video>'
                    tweet_info['media_files'].append({
                        'type': 'video',
